...
class Encoder(nn.Module):
    def __init__(self, config, num_classes, vis, early_exit_th):
        ...
        self.num_layers = config.transformer["num_layers"] - 1
    ...
    def forward(self, hidden_states):
        ...
        if self.training:
            ...
            return attn_weights, torch.stack(last_classifier), all_lte_output
...
class VisionTransformer(nn.Module):
    def __init__(self, config, img_size=224, num_classes=21843, zero_head=False, vis=False, early_exit_th=0.2, train_strategy='continuous-lte'):
    ...
    def forward(self, x, labels=None, step_num=-1):
        if labels is not None:
            ...
            if self.train_strategy == 'continuous-lte':
                ...
                loss = loss + 0.1*sum(all_early_exit_loss[:-1])/len(all_early_exit_loss) + lte_loss_fct(exit_pred, exit_label)  # classifier + lte
                # if step_num % 2 == 0:
                #     loss = loss + lte_loss_fct(exit_pred, exit_label) #vit + lte
                #     #loss = loss # vit
                # else:
                #     #loss = sum(all_early_exit_loss[:-1]) + lte_loss_fct(exit_pred, exit_label) #classifier + lte
                #     loss = loss + lte_loss_fct(exit_pred, exit_label) #vit + lte
                #     #loss = loss

            return loss, logits
        else:
            attn_weights, logits, all_lte_output = self.transformer(x)
            return logits
